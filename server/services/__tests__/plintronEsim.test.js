...
const mockPlintronConfig = {
  apiKey: 'TEST_API_KEY',
};

const mockPlintronRequest = jest.fn();

jest.unstable_mockModule('@config/esim.js', () => ({
  __esModule: true,

  PLINTRON: mockPlintronConfig,

  __setPlintronApiKey: (val) => {
    mockPlintronConfig.apiKey = val;
  },
}));

jest.unstable_mockModule('@utils/plintronClient.js', () => ({
  __esModule: true,
  plintronRequest: mockPlintronRequest,
}));

const { __setPlintronApiKey } = await import('@config/esim.js');
...
const {
  ...
  provisionEsimPack,
  fetchEsimUsage,
} = await import('@providers/plintronEsim.js');
...
describe('plintronEsim', () => {
  ...
  describe('ensureConfigured / missing API key', () => {
    ...
    it('activateProfile throws when PLINTRON not configured', async () => {
      await expect(
        activateProfile({ providerProfileId: 'p' })
      ).rejects.toHaveProperty(
        'code',
        'PLINTRON_NOT_CONFIGURED'
      );
    ...
    it('suspendLine throws when PLINTRON not configured', async () => {
      await expect(
        suspendLine({ providerProfileId: 'p' })
      ).rejects.toHaveProperty(
        'code',
        'PLINTRON_NOT_CONFIGURED'
      );
    ...
    it('resumeLine throws when PLINTRON not configured', async () => {
      await expect(
        resumeLine({ providerProfileId: 'p' })
      ).rejects.toHaveProperty(
        'code',
        'PLINTRON_NOT_CONFIGURED'
      );
  ...
  describe('reserveEsimProfile', () => {
    it('calls plintronRequest and maps response fields', async () => {
      ...
      mockPlintronRequest.mockResolvedValue(mockData);
      ...
      expect(mockPlintronRequest).toHaveBeenCalledTimes(1);

      expect(mockPlintronRequest).toHaveBeenCalledWith(
        '/esim/reserve',
        {
          method: 'POST',
  ...
  describe('activateProfile', () => {
    ...
    it('sets activatedAt to Date() when provider does not return activatedAt', async () => {
      mockPlintronRequest.mockResolvedValue({
        ok: true,
        msisdn: '+1555',
      });
      ...
      expect(res.providerMeta).toEqual({
        ok: true,
        msisdn: '+1555',
      });
    });
